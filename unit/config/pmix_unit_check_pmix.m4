...
AC_DEFUN([PMIXUNIT_CHECK_PMIX],[
    ...
    AS_IF([test "$with_pmix" = "no"],
          [AC_MSG_WARN([PMIx Unit tests require PMIx support using])
           AC_MSG_WARN([an external copy that you supply.])
           AC_MSG_ERROR([Cannot continue])])

    pmixunit_external_have_pmix1=0
    ...
    PMIXUNIT_CHECK_WITHDIR([pmix], [$pmix_ext_install_dir/include], [pmix.h])
    ...
    # check the version
    pmixunit_external_pmix_save_CPPFLAGS=$CPPFLAGS
    pmixunit_external_pmix_save_LDFLAGS=$LDFLAGS
    pmixunit_external_pmix_save_LIBS=$LIBS
    ...
    AS_IF([test "x`ls $pmix_ext_install_dir/include/pmix_version.h 2> /dev/null`" = "x"],
           [AC_MSG_RESULT([not found - assuming pre-v2.0])
            AC_MSG_WARN([PMIx Unit tests do not support PMIx versions])
            AC_MSG_WARN([less than v3.0])
            AC_MSG_ERROR([Please select a newer version and configure again])],
           [AC_MSG_RESULT([found])
            pmixunit_external_pmix_version_found=0])

    # if it does exist, then we need to parse it to find
    # the actual release series
    AS_IF([test "$pmixunit_external_pmix_version_found" = "0"],
          [AC_MSG_CHECKING([version 4x])
            AC_PREPROC_IFELSE([AC_LANG_PROGRAM([
                              ...
                              [AC_MSG_RESULT([found])
                               pmixunit_external_pmix_version=4x
                               pmixunit_external_pmix_version_found=4
                               pmixunit_prun_happy=yes],
                              [AC_MSG_RESULT([not found])])])
    ...
    AS_IF([test "$pmixunit_external_pmix_version_found" = "0"],
          [AC_MSG_CHECKING([version 3x])
            AC_PREPROC_IFELSE([AC_LANG_PROGRAM([
                              ...
                              [AC_MSG_RESULT([found])
                               pmixunit_external_pmix_version=3x
                               pmixunit_external_pmix_version_found=3
                               pmixunit_prun_happy=yes],
                              [AC_MSG_RESULT([not found])])])
    ...
    AS_IF([test "x$pmixunit_external_pmix_version" = "x"],
          [AC_MSG_WARN([PMIx version information could not])
           AC_MSG_WARN([be detected])
           AC_MSG_ERROR([cannot continue])])

    CPPFLAGS=$pmixunit_external_pmix_save_CPPFLAGS
    LDFLAGS=$pmixunit_external_pmix_save_LDFLAGS
    LIBS=$pmixunit_external_pmix_save_LIBS
