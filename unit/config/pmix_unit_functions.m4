...
#
# See if we can find an old installation of PMIX_UNIT to overwrite
#

# Stupid autoconf 2.54 has a bug in AC_PREFIX_PROGRAM -- if pmix_unit_clean
# is not found in the path and the user did not specify --prefix,
# we'll get a $prefix of "."

pmix_unit_prefix_save="$prefix"
AC_PREFIX_PROGRAM(pmix_unit_clean)
if test "$prefix" = "."; then
    prefix="$pmix_unit_prefix_save"
fi
unset pmix_unit_prefix_save
...
if test "$program_prefix$program_suffix$program_transform_name" != "NONENONEs,x,x," ; then
    AC_MSG_WARN([*** The PMIx_unit configure script does not support --program-prefix, --program-suffix or --program-transform-name. Users are recommended to instead use --prefix with a unique directory and make symbolic links as desired for renaming.])
...
for val in ${$1}; do
    ...
    pmix_unit_done="`expr $pmix_unit_i \> $pmix_unit_count`"
    while test "$pmix_unit_found" = "0" && test "$pmix_unit_done" = "0"; do

	# Have we seen this token already?  Prefix the comparison with
	# "x" so that "-Lfoo" values won't be cause an error.

	pmix_unit_eval="expr x$val = x\$pmix_unit_array_$pmix_unit_i"
	pmix_unit_found=`eval $pmix_unit_eval`
	...
	pmix_unit_i="`expr $pmix_unit_i + 1`"
    done
    ...
    -Xclang|-Xg)
            pmix_unit_found=0
            pmix_unit_i=`expr $pmix_unit_count + 1`
            ;;
    esac

    # If we didn't find the token, add it to the "array"

    if test "$pmix_unit_found" = "0"; then
	pmix_unit_eval="pmix_unit_array_$pmix_unit_i=$val"
	eval $pmix_unit_eval
	pmix_unit_count="`expr $pmix_unit_count + 1`"
    else
	pmix_unit_i="`expr $pmix_unit_i - 1`"
    fi
done
...
pmix_unit_done="`expr $pmix_unit_i \> $pmix_unit_count`"
pmix_unit_newval=
while test "$pmix_unit_done" = "0"; do
    pmix_unit_eval="pmix_unit_newval=\"$pmix_unit_newval \$pmix_unit_array_$pmix_unit_i\""
    eval $pmix_unit_eval

    pmix_unit_eval="unset pmix_unit_array_$pmix_unit_i"
    eval $pmix_unit_eval
...
pmix_unit_newval="`echo $pmix_unit_newval`"
pmix_unit_eval="$pmix_unit_name=\"$pmix_unit_newval\""
eval $pmix_unit_eval

# Clean up

unset pmix_unit_name pmix_unit_i pmix_unit_done pmix_unit_newval pmix_unit_eval pmix_unit_count])dnl
...
AC_DEFUN([PMIX_UNIT_FLAGS_UNIQ],[
    ...
    unset pmix_unit_name pmix_unit_i pmix_unit_done pmix_unit_newval pmix_unit_eval pmix_unit_count
])dnl
...
AC_DEFUN([PMIX_UNIT_FLAGS_APPEND_UNIQ], [
    ...
    for arg in $2; do
        ...
        pmix_unit_append=1
        AS_IF([test "$pmix_unit_tmp" = "-I" || test "$pmix_unit_tmp" = "-L" || test "$pmix_unit_tmp" = "-l"],
              [for val in ${$1}; do
                   AS_IF([test "x$val" = "x$arg"], [pmix_unit_append=0])
               done])
        AS_IF([test "$pmix_unit_append" = "1"],
...
IFS_SAVE=$IFS
IFS="$PATH_SEPARATOR"
for pmix_unit_dir in $PATH; do
    if test -x "$pmix_unit_dir/$pmix_unit_prog"; then
        $2="$pmix_unit_dir/$pmix_unit_prog"
        break
    fi
done
IFS=$IFS_SAVE
...
AC_DEFUN([PMIX_UNIT_VAR_SCOPE_PUSH],[
    ...
    for pmix_unit_var in $1; do
        pmix_unit_str="pmix_unit_str=\"\$$pmix_unit_var\""
        eval $pmix_unit_str
    ...
    pmix_unit_str="pmix_unit_scope_$pmix_unit_scope_index=\"$1\""
    eval $pmix_unit_str
    unset pmix_unit_str

    env | grep pmix_unit_scope
...
AC_DEFUN([PMIX_UNIT_VAR_SCOPE_POP],[
    ...
    pmix_unit_str="pmix_unit_str=\"\$pmix_unit_scope_$pmix_unit_scope_index\""
    eval $pmix_unit_str

    # Iterate over all the variables and unset them all
    for pmix_unit_var in $pmix_unit_str; do
        unset $pmix_unit_var
    done
])dnl
...
AC_DEFUN([PMIX_UNIT_COMPUTE_MAX_VALUE], [
    ...
    pmix_unit_num_bits=`expr $1 \* 8 - 1`
    newval=1
    value=1
    overflow=0

    while test $pmix_unit_num_bits -ne 0 ; do
        newval=`expr $value \* 2`
        if test 0 -eq `expr $newval \< 0` ; then
            # if the new value is not negative, next iteration...
            value=$newval
            pmix_unit_num_bits=`expr $pmix_unit_num_bits - 1`
            ...
            # below...)
            if test 0 -eq $pmix_unit_num_bits ; then
                value=`expr $value - 1`
            fi
        else
            ...
            if test 0 -eq `expr $newval \< 0` ; then
                ...
                # pmix_unit_num_bits is 1, we didn't actually overflow.
                # Otherwise, we overflowed.
                if test 1 -ne $pmix_unit_num_bits ; then
                    overflow=1
                fi
            else
